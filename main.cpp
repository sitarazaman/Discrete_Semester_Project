...
// ---------------- Module 5 SETS ------------------
    ...
    // functions and logic engine

    const int MAX_C = 50;
    string allCourses[MAX_C];
    string allPrereqs[MAX_C];
    int allCourseCount = 0;

    // --- Add courses to CourseSchedule and our arrays ---
    // Example additions (do this for every course you add)
    cs.addCourse("CS101", "");
    allCourses[allCourseCount] = "CS101";
    allPrereqs[allCourseCount] = "";
    allCourseCount++;

    cs.addCourse("CS102", "CS101");
    allCourses[allCourseCount] = "CS102";
    allPrereqs[allCourseCount] = "CS101";
    allCourseCount++;

    cs.addCourse("CS103", "CS102");
    allCourses[allCourseCount] = "CS103";
    allPrereqs[allCourseCount] = "CS102";
    allCourseCount++;

    cs.addCourse("Math101", "");
    allCourses[allCourseCount] = "Math101";
    allPrereqs[allCourseCount] = "";
    allCourseCount++;
    ...
    Student** list = sg.getAllStudents();
    int stuCount = sg.getStudentCount();

    for (int i = 0; i < stuCount; i++) {
        string courseForStudent;
        cout << "Assign a course to student " << list[i]->getName() << " (for functions demo): ";
    ...
    cout << endl;
    cout << "=== Functions Module Reports ===" << endl;
    fm.printStudentCourseMappings();
    fm.printCourseFacultyMappings();
    fm.printFacultyRoomMappings();

    bool courseInjective = fm.isInjectiveCourseToFaculty();
    cout << "Dry-run: Every course assigned to exactly one faculty? ";
    cout << (courseInjective ? "Yes" : "No") << endl;

    const int MAX_OUT = 200;
    string outStudents[MAX_OUT];
    string outRooms[MAX_OUT];
    ...
    if (stuCount > 0) {
        ...
        prover.proveStudentPrereqsForCourse(first->getName(), first->getCompletedCourses(), first->getCompletedCount(), "CS102", cs);
    }
